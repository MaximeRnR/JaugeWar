...
function registerUsername() {
  ...
  const jaugeWarUUID = v4();
  localStorage.jaugeWarUsername = inputUserName.value;
  localStorage.jaugeWarUUID = jaugeWarUUID;
  loggedIn.value = true;
  loggedPlayer.value = {id: userUUID, username: inputUserName.value, victory: 0};
  playersService?.addPlayer({id: jaugeWarUUID, username: inputUserName.value, victory: 0}).then(() => {
    players.value.push({id: userUUID, username: inputUserName.value, victory: 0});
...
onMounted(() => {
  if (localStorage.jaugeWarUUID && localStorage.jaugeWarUsername) {
    loggedPlayer.value = {id: localStorage.jaugeWarUUID, username: localStorage.jaugeWarUsername, victory: 0}
    loggedIn.value = true;
  }
...
<template>
  <div
      class="p-[8px] relative flex flex-col text-white gap-[12px] items-center w-[25%] sm:w-full bg-black/25 rounded-lg p-[8px]">
    <div v-if="!loggedIn" class="w-full flex flex-col gap-[4px]">
      ...
      <button
          class="flex justify-center items-center border border-solid border-white border-[4px] p-[4px] text-lg font-semibold rounded-lg"
          v-on:click="registerUsername()">Connexion
      </button>
    </div>
    <div v-if="loggedIn" class="w-full flex flex-col gap-[12px]">
      <div class="flex justify-between items-center gap-[2px]">
        ...
        <span class="text-lg font-semibold" v-if="loggedIn && loggedPlayer" v-on:keyup.enter="registerUsername()">{{
      ...
      <div class="flex flex-col gap-[8px]">
        ...
        <div class="w-full flex flex-row justify-between items-center gap-[4px]">
          <input v-model="roomName" type="text" id="roomName" v-on:keyup.enter="createGame()"
                 class="w-[70%] rounded-lg flex p-[8px] text-black" placeholder="Nom de la partie">
          <button
              class="w-[20%] flex justify-center items-center border border-solid border-white border-[4px] p-[4px] text-lg font-semibold rounded-lg"
              v-on:click="createGame()">Créer
          </button>
        </div>
      </div>
      <div class="flex flex-col gap-[8px]">
        ...
        <div v-for="game in games"
             class="flex justify-between items-center p-[8px] bg-white/50 rounded-lg max-h-[20vh] overflow-auto">
          <span class="text-lg font-bold">{{ game.name }}</span>
          <span class="text-lg font-bold">{{ game.owner.username }}</span>
          <button
              class="text-lg font-semibold flex justify-center items-center border border-solid border-white border-[4px] p-[4px] text-lg font-semibold rounded-lg"
              v-on:click="joinGame(game.id)"> Rejoindre
          </button>
        </div>
      </div>
    </div>
